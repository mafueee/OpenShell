...
mod tests {
    ...
    #[test]
    fn discovers_ollama_env_credentials() {
        let ctx = MockDiscoveryContext::new().with_env("OLLAMA_API_KEY", "ollama-test-key");
        let discovered = discover_with_spec(&SPEC, &ctx)
            .expect("discovery")
            .expect("provider");
        assert_eq!(
            discovered.credentials.get("OLLAMA_API_KEY"),
            Some(&"ollama-test-key".to_string())
        );
    }
}
